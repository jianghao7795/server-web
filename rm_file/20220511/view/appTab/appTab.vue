<template>
  <div>
    ...
    <div class="gva-table-box">
        ...
        <el-table-column align="left" label="管理ID" prop="sysUserId" width="120">
            <template #default="scope">{{ formatBoolean(scope.row.sysUserId) }}</template>
        </el-table-column>
        <el-table-column align="left" label="按钮组">
            <template #default="scope">
            <el-button type="text" icon="edit" size="small" class="table-button" @click="updateAppTabFunc(scope.row)">变更</el-button>
...
const onSubmit = () => {
  page.value = 1
  pageSize.value = 10
  if (searchInfo.value.sysUserId === ""){
      searchInfo.value.sysUserId=null
  }
  getTableData()
}
...
const getTableData = async() => {
  const table = await getAppTabList({ page: page.value, pageSize: pageSize.value, ...searchInfo.value })
  if (table.code === 0) {
...
// 删除行
const deleteAppTabFunc = async (row) => {
