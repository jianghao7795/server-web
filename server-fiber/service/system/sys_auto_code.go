...
func (autoCodeService *AutoCodeService) CreatePackageTemp(packageName string) error {
	...
	for _, v := range pendingTemp {
		...
		if err := ImportReference(meta.path, fmt.Sprintf(meta.importCodeF, v.name, packageName), fmt.Sprintf(meta.structNameF, cases.Title(language.Dutch).String(packageName)), fmt.Sprintf(meta.packageNameF, packageName), meta.groupName); err != nil {
			return err
		}
	}
	return nil
			...
			Rhs: []ast.Expr{
				&ast.SelectorExpr{
					...
					Sel: &ast.Ident{
						...
						Name: cases.Title(language.Dutch).String(vi.PackageName),
					},
				},
			},
		}
